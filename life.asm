...
LCD_INITIALIZE proc near

  mov al,30h 
  out officer_porta, al  ;function set : 1lin, 8-bit : 16 characters on a line. 5x7 dots used.
;clr lcd_rs        ;for an instruction command, rs and rw both 0
;clr lcd_rw
;setb lcd_en     ; H-> L transition needed for enabling
  mov al, 00100000b  
  out officer_portb, al

;clr lcd_en
  mov al, 00000000b
  out officer_portb, al

  call BUSY    ;for smooth executioning and nothing else happening

  mov al, 0fh  
  out officer_porta, al    ;display on, cursor blinking
  mov al, 00100000b
  out officer_portb, al
  mov al, 00000000b
  out officer_portb, al
  ...
  mov al, 01h  
  out officer_porta, al     ;clear the lcd
  ...
  mov al, 06h 
  out officer_porta, al    ;entry mode. Auto increment with no shift.
  mov al, 00100000b
  out officer_portb, al
  mov al, 00000000b
  out officer_portb, al
...
;setb LCD_rw ;we are reading
  ...
  mov al, 10000000b
  out portA_address_82551, al
  mov al, 01100000b
  out portB_address_82551, al
...
;setb LCD_en
  ...
  in al,  portA_address_82551
  and al,80h
  cmp al,80h
  jz check ;read busy flag again and again till it becomes 0
